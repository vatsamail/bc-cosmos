...
export interface MsgCheckers {
  creator: string;
}

export interface MsgCheckersResponse {}
...
const baseMsgCheckers: object = { creator: "" };

export const MsgCheckers = {
  encode(message: MsgCheckers, writer: Writer = Writer.create()): Writer {
    if (message.creator !== "") {
      writer.uint32(10).string(message.creator);
    }
    return writer;
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgCheckers {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseMsgCheckers } as MsgCheckers;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): MsgCheckers {
    const message = { ...baseMsgCheckers } as MsgCheckers;
    if (object.creator !== undefined && object.creator !== null) {
      message.creator = String(object.creator);
    } else {
  ...
  toJSON(message: MsgCheckers): unknown {
    const obj: any = {};
    message.creator !== undefined && (obj.creator = message.creator);
    return obj;
  },

  fromPartial(object: DeepPartial<MsgCheckers>): MsgCheckers {
    const message = { ...baseMsgCheckers } as MsgCheckers;
    if (object.creator !== undefined && object.creator !== null) {
      message.creator = object.creator;
    } else {
...
const baseMsgCheckersResponse: object = {};

export const MsgCheckersResponse = {
  encode(_: MsgCheckersResponse, writer: Writer = Writer.create()): Writer {
    return writer;
  },

  decode(input: Reader | Uint8Array, length?: number): MsgCheckersResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseMsgCheckersResponse } as MsgCheckersResponse;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        default:
  ...
  fromJSON(_: any): MsgCheckersResponse {
    const message = { ...baseMsgCheckersResponse } as MsgCheckersResponse;
    return message;
  },

  toJSON(_: MsgCheckersResponse): unknown {
    const obj: any = {};
    return obj;
  },

  fromPartial(_: DeepPartial<MsgCheckersResponse>): MsgCheckersResponse {
    const message = { ...baseMsgCheckersResponse } as MsgCheckersResponse;
    return message;
  },
};
...
export interface Msg {
  ...
  Checkers(request: MsgCheckers): Promise<MsgCheckersResponse>;
